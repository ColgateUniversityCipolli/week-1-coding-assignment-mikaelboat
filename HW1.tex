...
\hlkwd{canvas_collatz}\hldef{(}\hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"tuscany1"}\hldef{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlkwd{canvas_collatz}\hldef{(}
  ...
  \hlkwc{angle.odd} \hldef{=} \hlopt{-}\hlnum{0.05}
\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlkwc{background} \hldef{=} \hlsng{"#ffffff"}\hldef{,}
\hlkwc{n} \hldef{=} \hlnum{150}\hldef{,}
\hlkwc{angle.odd} \hldef{=} \hlnum{0.45}\hldef{,}
\hlkwc{angle.even} \hldef{=} \hlnum{0.25}
\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{vect} \hlkwb{<-} \hlkwd{seq}\hldef{(}\hlkwc{from}\hldef{=}\hlnum{0}\hldef{,} \hlkwc{to}\hldef{=}\hlnum{10}\hldef{,} \hlkwc{by}\hldef{=}\hlnum{1}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{answers} \hlkwb{<-} \hlkwd{numeric}\hldef{(}\hlkwc{length} \hldef{=} \hlnum{10}\hldef{)}
\hldef{j} \hlkwb{<-} \hlnum{1}
\hlkwa{for} \hldef{(i} \hlkwa{in} \hldef{vect) \{}
  \hldef{answers[j]} \hlkwb{<-} \hldef{(}\hlnum{2}\hlopt{^}\hldef{(i}\hlopt{+}\hlnum{1}\hldef{)} \hlopt{+} \hlnum{2}\hlopt{^}\hldef{(i}\hlopt{-}\hlnum{1}\hldef{))}  \hlcom{# Add the missing closing parenthesis here}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{which}\hldef{(answers} \hlopt{==} \hlnum{40}\hldef{)}
\end{alltt}
\begin{verbatim}
## [1] 5
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{vect[}\hlkwd{which}\hldef{(answers} \hlopt{==} \hlnum{40}\hldef{)]}
\end{alltt}
\begin{verbatim}
## [1] 4
\end{verbatim}
...
\textbf{Solution:} This method is dependent on evaluating the problem for whole integer values. Because, $3^{x+2} + 5(3^x) = 84$ has a decimal solution, this approach wouldn't work.
\end{enumerate}
\end{enumerate}
